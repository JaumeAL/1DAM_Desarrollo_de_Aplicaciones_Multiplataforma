...
--EXEMPLARS NEGATIUS = 0
CREATE OR REPLACE TRIGGER before_insert_exemplars
    BEFORE INSERT ON LLIBRE
    FOR EACH ROW
BEGIN
    IF :NEW.exemplars < 0 THEN -- :NEW.exemparrs es como una variable
        :NEW.exemplars := 0;
    END IF;
END;

-- NUEVAS EDITORIALES CON NOMBRE EN MAYUS
CREATE OR REPLACE TRIGGER before_insert_or_update_editorial
    BEFORE INSERT OR UPDATE EDITORIAL
    FOR EACH ROW
BEGIN
...
--SIN EDITORIAL
CREATE OR REPLACE TRIGGER before_insert_llibre
    BEFORE INSERT ON LLIBRE
    FOR EACH ROW
DECLARE
...
BEGIN
    IF :NEW.id is NULL THEN
        ...
        :NEW.id:=v_id_editorial;
    END IF;
END;

--CREAR LLIBRE SECUALA
CREATE OR REPLACE TRIGGER before_insert_llibre_sequela
    AFTER INSERT ON LLIBRE
    FOR EACH ROW
BEGIN
    IF :NEW.ID_sequela_de is not null THEN
        for gen in (select nom_genere from llibre-genere where id_llibre = :new.id_squela_de) loop
        insert into llibre_genere (id_llibre, nom_genere) values 
        (:new.id,gen.nom_genere);
    end loop;
    for aut in...


--CREAR LIBRO GENERO MAS FRECUENTE
CREATE OR REPLACE TRIGGER before_insert_genero_frecuente
    AFTER INSERT ON AUTOR_LLIBRE
    FOR EACH ROW
BEGIN
    IF :NEW.ID_autor