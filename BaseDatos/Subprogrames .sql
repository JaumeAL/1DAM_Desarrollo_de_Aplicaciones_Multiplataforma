...
declare
    ...
    v_autor_id number := 2;
begin
    select * bulk collect into llibres from LLIBRE where ID in (select ID_llibre from AUTOR_LLIBRE where ID_autor = v_autor_id);
    
    for i in 1..llibres.count loop
        dbms_output.put_line('titulo:' || llibres(i).titol);
    end loop;
end;
...
declare
    v_id number := 1;
    v_titol varchar2(50);
    v_titol_actualitzat varchar2(50);
begin
    update LLIBRE set titol = v_titol where ID = v_id returning titol into v_titol_actualitzat;
    dbms_output.put_line('titulo actualiizado:' || v_titol_actualitzat);
exception
    when no_data_found then
        dbms_output.put_line('no hay ningun libro con ID:' || v_id);
end;
...
declare
    ...
    pragma exception_init(no_data_found, -942);
begin
    update LLIBRE set titol = v_titol where ID = v_id returning titol into v_titol_actualitzat;
    dbms_output.put_line('titulo actualizado:' || v_titol_actualitzat);
exception
    when no_data_found then
        dbms_output.put_line('no hay ningun libro con ID:' || v_id);
        raise no_data_found;
